...
### Tooling
`ruff check src/` + `ruff format --check src/` clean. `pyright src/` byte-identical to the pre-merge baseline (6 pre-existing errors: `reportImplicitStringConcatenation` ×4, `reportUnusedFunction` ×2 on `_is_blocked_video_only_extension` / `_build_sidecar_map` — pyright treats the `kodi://sources` loop as unreachable after upstream's `media = KodiMediaTypes.VIDEOS.value` narrowing, so it provides **zero** type coverage of that site). Line endings kept LF (a Windows text-mode rewrite briefly flipped `media_browser.py` and `driver.json` to CRLF — caught by the whole-file diff and normalized before commit). Built with the standard PyInstaller container command; tarball `kodi-integration-v1.21.0-madalone.1.tar.gz`.
...
- **V8** `pvr://recordings` via a favourite (routes to the sources loop): renders, no crash.
